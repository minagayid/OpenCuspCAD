...
The current approval gate fingerprints a saved proposal and creates a local STL/OBJ plus JSON CAM handoff manifest only after reviewer and machine-profile fields are present. It does not transmit to a machine or create a toolpath. The next engineering gates are a verified scan registration record, an editable three-dimensional margin, an insertion-axis and undercut report, separate intaglio zones with measured clearances, signed occlusion/contact maps, material and machine profiles, and machine-specific CAM validation. Each gate needs dental-lab review cases and documented acceptance tolerances before it can be enabled for manufacturing output.
